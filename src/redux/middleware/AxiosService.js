...
const getAxiosProps = ({
                           method,
                           baseURL = window.process.env.REACT_APP_BACKEND_BASE_URL,
                           url,
                           payload,
                           header = {}
                       }) => ({
...
const AxiosService = ({getState}) => next => async action => {
    const {type = 'GET', payload, header, method, baseURL, url, axiosService, toaster = {}} = action;
    ...
    if (axiosService) {
        ...
        try {
            ...
            if (success)
                toast.success(isString(success) ? success : response.data.message, {id: loaderId});
            else
            ...
            if (error)
                toast.error((isString(error) ? error : axiosError?.response?.data?.error) || 'Something went to wrong !', {id: loaderId});
            else
                toast.dismiss(loaderId);
